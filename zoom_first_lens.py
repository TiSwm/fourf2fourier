...
def get_model(shift_obj=0, shift_tube=0, NA=0.04):
    ...
    sm.add_surface([-197.7, 172.07 - shift_tube])
...
def mtf_guess(sm):
    pitches = np.linspace(5, 200, 20) * 1e-3
    contrast = np.zeros_like(pitches)
    for i, p in enumerate(pitches):
        contrast[i] = calc_contrast(sm, p, 0.6 / 5, 0.04)
    return pitches, contrast


shifts = np.linspace(-1, 1, 20)
